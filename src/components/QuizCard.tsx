...
export function QuizCard({ question, showHint, onSpeak }: Props) {
  const thToEn = question.type === "thToEn";
  return (
    <article className="relative overflow-hidden rounded-3xl border border-white bg-white/95 p-4 shadow-lg shadow-blue-950/5">
      ...
      <div className="relative flex items-start justify-between gap-2">
        <button type="button" className="min-w-0 flex-1 text-left" onClick={onSpeak} disabled={!onSpeak}>
          <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
            {thToEn ? "เลือกคำอังกฤษ (English)" : "คำศัพท์พื้นฐาน (Vocabulary)"}
          </span>
