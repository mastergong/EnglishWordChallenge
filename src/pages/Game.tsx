...
function GamePlay() {
  ...
  return (
    <div className="font-game relative mx-auto flex h-dvh max-w-[420px] flex-col overflow-hidden bg-gradient-to-b from-[#FFFEFC] via-[#FFF8FB] to-[#F5F9FF] text-slate-800">
      <div className="game-blob pointer-events-none absolute -left-16 -top-16 h-56 w-56 rounded-full bg-rose-300/40" />
      ...
      <header className="relative z-10 shrink-0 px-5 pb-2 pt-[max(1rem,env(safe-area-inset-top))]">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <div className="min-w-0 flex-1 px-1">
            <div className="mb-1.5 flex items-center justify-between px-0.5 text-xs font-bold text-slate-500">
              <span className="font-game-display text-rose-600">
            ...
            <div className="h-3.5 overflow-hidden rounded-full bg-slate-200/80 p-0.5 shadow-inner">
              <div
                className="relative h-full rounded-full bg-gradient-to-r from-amber-400 via-rose-500 to-pink-500 transition-all duration-500"
                style={{ width: `${percent}%` }}
              >
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <div className="inline-flex min-w-0 items-center gap-1.5 rounded-full border border-rose-200 bg-rose-100/80 px-3 py-1 text-xs font-bold tracking-wide text-rose-700 shadow-sm">
            ...
            <p className="px-1 pb-1 text-center text-sm font-bold" aria-live="polite">
              {last.outcome === "correct"
                ...
                : last.outcome === "timeout"
                  ? "หมดเวลาแล้ว ไม่เป็นไรนะ"
                  : "ยังไม่ถูก ลองจำความหมายนี้ไว้"}{" "}
      ...
      <footer className="relative z-10 shrink-0 border-t border-rose-100 bg-white/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_-8px_20px_rgb(251_113_133/0.06)] backdrop-blur-md">
        <div className="mb-2 flex items-center justify-center gap-3">
          <button
            type="button"
            disabled={fiftyUsed || phase !== "question"}
            onClick={useFiftyFifty}
            className="flex items-center gap-1.5 rounded-xl bg-rose-50 px-3 py-1.5 text-xs font-bold text-rose-600 disabled:opacity-40"
          >
            <span className="font-game-display text-indigo-600">⚡</span>
            50:50
          </button>
          <button
            type="button"
            ...
            className="flex items-center gap-1.5 rounded-xl border border-rose-200/70 bg-rose-50 px-3 py-1.5 text-xs font-bold text-rose-600 disabled:opacity-40"
          >
            <span>💡</span>
            คำใบ้
          </button>
        ...
        {phase === "feedback" && !settings.autoNext && !quiz.finished ? (
          <button
            type="button"
            onClick={advance}
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-b from-rose-400 to-rose-500 py-3.5 text-lg font-extrabold tracking-wide text-white shadow-[0_5px_0_0_#e11d48] active:translate-y-1 active:shadow-none"
          >
            ข้อถัดไป
          </button>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-b from-rose-400 to-rose-500 py-3.5 text-lg font-extrabold tracking-wide text-white shadow-[0_5px_0_0_#e11d48] disabled:opacity-40 active:translate-y-1 active:shadow-none"
          >
            ส่งคำตอบ (Submit)
