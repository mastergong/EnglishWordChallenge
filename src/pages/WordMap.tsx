  ...
  return (
    <div className="min-w-0 space-y-3 overflow-x-hidden">
          ...
          {families.map((item) => (
            <button
              ...
              className={`min-h-10 shrink-0 rounded-full px-3 text-sm font-semibold ${
                item.key === family.key ? "bg-indigo-600 text-white" : "bg-white/80 dark:bg-white/10"
              }`}
            >
      ...
      <section className="rounded-3xl bg-[#1e1b4b] p-2">
        <p className="px-2 pb-1 text-center text-[11px] text-indigo-200">
              ...
              <li key={word.id}>
                <button
                  ...
                  className={`flex min-h-12 w-full items-start justify-between gap-3 rounded-2xl px-3 py-2 text-left ${
                    active ? "bg-indigo-600 text-white" : "bg-white/90 dark:bg-white/5"
                  }`}
                >
                  <span>
                    <span className="block font-black">{word.word}</span>
                    <span className={`block text-xs ${active ? "text-indigo-100" : "text-sky-600 dark:text-sky-300"}`}>
