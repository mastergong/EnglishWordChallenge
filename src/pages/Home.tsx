  ...
  return (
    <div className="space-y-5">
      <header className="pt-2">
        <p className="text-sm font-semibold text-blue-500">Mobile vocabulary game</p>
        <h1 className="text-4xl font-black leading-tight">
          English Word
          <span className="block text-blue-600 dark:text-blue-300">Challenge</span>
        </h1>
      ...
      <Link
        to="/daily"
        className="block rounded-3xl bg-blue-600 p-5 text-white shadow-lg"
      >
          ...
          {([10, 20, 30, 40, 50] as const).map((count) => (
            <button
              ...
              className={`min-h-11 rounded-2xl text-sm font-bold ${
                questionCount === count ? "bg-blue-600 text-white" : "bg-white/80 dark:bg-white/10"
              }`}
            >
              {count}
            </button>
      ...
      <button
        ...
        className="flex min-h-14 w-full items-center justify-center rounded-3xl bg-blue-600 text-xl font-black text-white shadow-lg"
      >
        ▶ START GAME · เริ่มเกม
      </button>
